...
public class SocketWRSkullAreaCloud extends AreaEffectCloud {
    private final int duration = 5;
    private final float radius = 2f;
    private final Map<Entity, Integer> victims = Maps.newHashMap();

    public SocketWRSkullAreaCloud(EntityType<? extends SocketWRSkullAreaCloud> type, Level world) {
        super(type, world);
    }

    public SocketWRSkullAreaCloud(Level world) {
        this(EntityTypeRegistry.SOCKET_WRSKULL_AREA_CLOUD.get(), world);
    }
    ...
    public void tick() {
        ...
        if (level().isClientSide) {
            for (int particles = 0; particles < 30; ++particles) {
                ...
                level().addAlwaysVisibleParticle(ParticleTypeRegistry.WRSKULL_SPHERE_PARTICLES.get(), getX() + x, getY() + radius + y, getZ() + z, x * multiplier, y * multiplier, z * multiplier);
                setNoGravity(true);
            }
        } else {
