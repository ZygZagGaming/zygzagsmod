...
public class BigShurikenAssembly extends Entity implements GeoEntity {
    ...
    private final AnimationController<BigShurikenAssembly> controller = new AnimationController<>(this, "main", 0, (animState) -> animState.setAndContinue(target == null ? IDLE : SPIN_UP));
    private final AnimatableInstanceCache instanceCache = GeckoLibUtil.createInstanceCache(this);
    private final boolean isFast, breaksOnHittingABlock;
    private int health, damage;

    public BigShurikenAssembly(EntityType<? extends BigShurikenAssembly> type, Level world) {
        this(type, world, false, false, 3, 4);
    }

    public BigShurikenAssembly(EntityType<? extends BigShurikenAssembly> type, Level world, boolean isFast, boolean breaksOnHittingABlock, int health, int damage) {
        super(type, world);
        this.isFast = isFast;
            ...
            if (!canTarget(target)) target = null;
                ...
                if (diff.lengthSqr() >= 0.05) {
                    if (isFast) {
                        ...
                        diff = diff.multiply(1, 0, 1).normalize().scale(0.05).add(diff.multiply(0, 0.15, 0));
