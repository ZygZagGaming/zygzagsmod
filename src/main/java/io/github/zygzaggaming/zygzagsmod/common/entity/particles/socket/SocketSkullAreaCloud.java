...
public class SocketSkullAreaCloud extends AreaEffectCloud {
    private final int duration = 7;
    private final float radius = 0.1f;
    private final Map<Entity, Integer> victims = Maps.newHashMap();

    public SocketSkullAreaCloud(EntityType<? extends SocketSkullAreaCloud> type, Level world) {
        super(type, world);
    }

    public SocketSkullAreaCloud(Level world) {
        this(EntityTypeRegistry.SOCKET_SKULL_AREA_CLOUD.get(), world);
    }
    ...
    public void tick() {
        ...
        if (level().isClientSide) {
            for (int particles = 0; particles < 15; ++particles) {
                ...
                level().addAlwaysVisibleParticle(ParticleTypeRegistry.SKULL_SPHERE_PARTICLES.get(), getX() + x, getY() + radius + y, getZ() + z, x * multiplier, y * multiplier, z * multiplier);
            }
        } else {
