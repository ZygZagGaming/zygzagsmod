...
public class HelixBAssembly extends Monster implements GeoAnimatable, ActingEntity<HelixBAssembly> {
    ...
    protected static final EntityDataAccessor<Optional<UUID>> DATA_TARGET = SynchedEntityData.defineId(HelixBAssembly.class, EntityDataSerializers.OPTIONAL_UUID);
    private @Nullable Player target = null;
    //private @Nullable LivingEntity target = null;
    private int health;
    private int chargeTime;

    public HelixBAssembly(Level world) {
        this(EntityTypeRegistry.HELIX_ASSEMBLY_B.get(), world, 3);
    }

    public HelixBAssembly(EntityType<? extends HelixBAssembly> type, Level world, int health) {
        super(type, world);
        this.health = health;
        setPathfindingMalus(PathType.WATER, -1);
        setPathfindingMalus(PathType.LAVA, 8);
        this.moveControl = new HelixBAssembly.RodMoveControl(this);
    }

    @Override
    protected void registerGoals() {
        this.goalSelector.addGoal(5, new HelixBAssembly.RandomFloatAroundGoal(this));
        ...
        targetSelector.addGoal(3, new HelixBAssembly.OsuNATGoal<>(this, LivingEntity.class, 1, true, false, (entity) -> (entity instanceof Player player && !player.isCreative() && !player.isSpectator()) || entity instanceof AbstractGolem));    }
    ...
    @Override
    public Actor<HelixBAssembly> getActor() {
        return actor;
    }
    ...
    static class RandomFloatAroundGoal extends Goal {
        private final HelixBAssembly helixBAssembly;

        public RandomFloatAroundGoal(HelixBAssembly p_32783_) {
            this.helixBAssembly = p_32783_;
            this.setFlags(EnumSet.of(Flag.MOVE));
        }

        @Override
        public boolean canUse() {
            MoveControl movecontrol = this.helixBAssembly.getMoveControl();
            if (!movecontrol.hasWanted()) {
                return true;
            } else {
                double d0 = movecontrol.getWantedX() - this.helixBAssembly.getX();
                double d1 = movecontrol.getWantedY() - this.helixBAssembly.getY();
                double d2 = movecontrol.getWantedZ() - this.helixBAssembly.getZ();
    ...
    static class RodMoveControl extends MoveControl {
        ...
        public RodMoveControl(HelixBAssembly p_32768_) {
            super(p_32768_);
            this.helixBAssembly = p_32768_;
        }
        ...
        public void tick() {
            if (this.operation == Operation.MOVE_TO) {
                if (this.floatDuration-- <= 0) {
                    this.floatDuration = this.floatDuration + this.helixBAssembly.getRandom().nextInt(5) + 2;
                    Vec3 vec3 = new Vec3(this.wantedX - this.helixBAssembly.getX(), this.wantedY - this.helixBAssembly.getY(), this.wantedZ - this.helixBAssembly.getZ());
                    double d0 = vec3.length();
                    vec3 = vec3.normalize();
                    if (this.canReach(vec3, Mth.ceil(d0))) {
                        this.helixBAssembly.setDeltaMovement(this.helixBAssembly.getDeltaMovement().add(vec3.scale(0.1)));
                    } else {
        ...
        private boolean canReach(Vec3 p_32771_, int p_32772_) {
            AABB aabb = this.helixBAssembly.getBoundingBox();

            for (int i = 1; i < p_32772_; i++) {
                aabb = aabb.move(p_32771_);
                if (!this.helixBAssembly.level().noCollision(this.helixBAssembly, aabb)) {
                    return false;
                }
            }
