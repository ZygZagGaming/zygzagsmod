...
public class ModUtil {
    public static void incrementEntityOverheat(LivingEntity entity, int increment) {
        if (increment != 0) {
            int old = entity.getData(AttachmentTypeRegistry.LIVING_ENTITY_OVERHEAT_ATTACHMENT.get());
            entity.setData(AttachmentTypeRegistry.LIVING_ENTITY_OVERHEAT_ATTACHMENT.get(), old + increment);
    ...
    public static void setEntityOverheat(LivingEntity entity, int newOverheat) {
        int old = entity.getData(AttachmentTypeRegistry.LIVING_ENTITY_OVERHEAT_ATTACHMENT.get());
        if (old != newOverheat) {
            entity.setData(AttachmentTypeRegistry.LIVING_ENTITY_OVERHEAT_ATTACHMENT.get(), newOverheat);
